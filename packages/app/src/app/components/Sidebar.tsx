...
const selected = {
  nav: {
    borderImageSource:
      "linear-gradient(91.57deg, rgba(255, 255, 255, 0.4) -4.29%, rgba(255, 255, 255, 0) 110.62%)",
    boxShadow:
      "5px 5px 10px 0px #FE460066 inset, -5px -5px 10px 0px #FAFBFFAD inset",
    iconColor: "hsl(var(--malachite))",
    color: "hsl(var(--primary))",
  },
  pending: {
    boxShadow:
      "5px 5px 10px 0px #FFC12166 inset, -5px -5px 10px 0px #FAFBFFAD inset",
  },
  active: {
    boxShadow:
      "5px 5px 10px 0px #FE460066 inset, -5px -5px 10px 0px #FAFBFFAD inset",
    ...
    <aside className="relative flex">
      ...
      >
        <section className="flex flex-col gap-4 max-h-56 pb-4 overflow-y-auto">
          {wallet ? (
            <button className="bg-primary rounded-full p-4 hover:opacity-70 flex justify-center items-center gap-2 text:white">
              Connected
            </button>
          ) : (
            <button className="bg-primary rounded-full p-4 hover:opacity-70 flex justify-center items-center gap-2 text:white" onClick={() => connectWallet()}>
              Connect Wallet
            </button>
          )}
        </section>
          ...
          {tabs.map((tab) => (
            <button
              ...
              style={{
                borderImageSource:
                  selectedTab === tab.id
                    ? selected.nav.borderImageSource
                    : unselected.nav.borderImageSource,
                boxShadow:
                  selectedTab === tab.id
                    ? selected.nav.boxShadow
                    : unselected.nav.boxShadow,
                color:
                  selectedTab === tab.id
                    ? selected.nav.color
                    : unselected.nav.color,
              }}
              onClick={() => setSelectedTab(tab.id)}
            >
              {tab.icon(
                selectedTab === tab.id
                  ? selected.nav.iconColor
                  : unselected.nav.iconColor
              )}
              {tab.name}
            </button>
          ))}
            ...
            {tasks.map((task) => (
              <button
                ...
                style={{
                  boxShadow:
                    selectedTab === task.id
                      ? selected.pending.boxShadow
                      : "none",
                }}
                onClick={() => setSelectedTab(task.id)}
              >
                <div className="flex items-center gap-2">
                  <Image src={pendingAvatar} alt="" width={40} height={40} />
                  <div className="flex flex-col items-start gap-0.5">
                    <p className="text-primary-foreground font-medium">
                      {task.taskType}
                    </p>
                    <p className="text-xs text-primary">assign agent</p>
                  </div>
                </div>
        ...
        <section className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-3 max-h-28 overflow-y-auto">
            {proposals
              .filter((proposal) => proposal.task.status === "assigned")
              .map((proposal) => (
                <button
                  key={proposal.id}
                  className="flex justify-between items-center gap-2 bg-background border border-primary-foreground p-1 max-w-48 rounded-full transition-all ease-in-out duration-300 hover:opacity-70"
        ...
        <section className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-3 max-h-28 overflow-y-auto">
            {proposals
              .filter((proposal) => proposal.task.status === "completed")
              .map((proposal) => (
                <div
                  key={proposal.id}
                ...
                >
                  ...
                  <Image src={check} alt="" width={32} height={32} />
                </div>
              ))}
          </div>
        </section>
